...
void receive_ping(t_env *env)
{
	...
	if (((uint32_t) received - env->headers_len) >= sizeof(size_t))
	{
		...
		if (diff > env->ping_max)
			env->ping_max = diff;
		env->ping_total += diff;
		//printf("%ld bytes from %s (%s): icmp_seq=%d ttl=%d time=%.1f ms", received - sizeof(env->receive_packet->ip_header), env->dst, env->addr, env->receive_packet->icmp_header.icmp_hun.ih_idseq.icd_seq, env->receive_packet->ip_header.ip_ttl, diff / 1000.f);
